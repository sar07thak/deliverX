...
public class DPRegistrationService : IDPRegistrationService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Resolve DPCM from referral code if provided
        Guid? dpcmId = null;
        if (!string.IsNullOrEmpty(request.DPCMReferralCode))
        {
            dpcmId = await ResolveDPCMFromReferralCodeAsync(request.DPCMReferralCode, ct);
            if (dpcmId.HasValue)
            {
                _logger.LogInformation("DP {UserId} linked to DPCM via referral code {ReferralCode}", userId, request.DPCMReferralCode);
            }
        }
        ...
        if (existingProfile != null)
        {
            ...
            // Set DPCMId if not already set and we have one from referral
            if (!existingProfile.DPCMId.HasValue && dpcmId.HasValue)
            {
                existingProfile.DPCMId = dpcmId;
            }
        }
        else
        {
            ...
            var profile = new DeliveryPartnerProfile
            {
                Id = Guid.NewGuid(),
                UserId = userId,
                DPCMId = dpcmId, // Link to DPCM if referral code was provided
    ...
    /// <summary>
    /// Resolves DPCM Id from referral code.
    /// Referral code format: DPCM{first 8 chars of UserId}
    /// Example: DPCM12345678 where 12345678 is the start of the DPCM's UserId
    /// </summary>
    private async Task<Guid?> ResolveDPCMFromReferralCodeAsync(string referralCode, CancellationToken ct)
    {
        if (string.IsNullOrEmpty(referralCode))
            return null;

        // Referral code format: DPCM followed by first 8 chars of UserId
        if (!referralCode.StartsWith("DPCM", StringComparison.OrdinalIgnoreCase) || referralCode.Length < 12)
        {
            _logger.LogWarning("Invalid DPCM referral code format: {ReferralCode}", referralCode);
            return null;
        }

        var userIdPrefix = referralCode.Substring(4).ToLower(); // Remove "DPCM" prefix

        // Find DPCM whose UserId starts with this prefix
        var dpcmManagers = await _context.DPCManagers
            .Where(d => d.IsActive)
            .ToListAsync(ct);

        var matchingDpcm = dpcmManagers
            .FirstOrDefault(d => d.UserId.ToString().Replace("-", "")
                .StartsWith(userIdPrefix, StringComparison.OrdinalIgnoreCase));

        if (matchingDpcm == null)
        {
            _logger.LogWarning("No active DPCM found for referral code: {ReferralCode}", referralCode);
            return null;
        }

        _logger.LogInformation("Resolved DPCM {DPCMId} from referral code {ReferralCode}", matchingDpcm.Id, referralCode);
        return matchingDpcm.Id;
    }
}
